...
# 4. 归一化：第N次取票环节 → 取票环节；反采同程ITravel → 反采同程
...
REASON_CLEANUP_RULES = [
    ...
    (r'""', ""),
    # 7) 归一化：第N次取票失败 → 取票环节（族名变了，reason 文本里的"取票失败"是变体关键词）
    (r"第\s*\d+\s*次取票失败", "取票环节"),
    (r"第\s*null\s*次取票失败", "取票环节"),
...
REASON_FAMILY_RULES = [
    # === v10.x（2026-07-26）："X环节"前缀匹配（必须在所有具体关键词规则之前）
    # 背景：实际数据里有大量 reason 文本以"取票环节 原因为:xxx" / "取票环节 当前订单状态为:xxx" 开头
    #   之前族规则只匹配"取票失败/支付失败"等具体关键词，957+ 单"取票环节"前缀 reason
    #   全兜底到"其他环节-兜底" → 8 大环节汇总严重失真
    # 修法：先按"X环节"前缀粗归类（覆盖 90% case），再用更具体的关键词规则精化（如"取票失败/验真异常"）
    (r"取票环节", "取票环节-取票异常"),
    (r"支付环节", "支付环节-支付异常"),
    (r"预定环节", "预定环节"),
    (r"验真环节", "验真环节-验真异常"),
    ...
    # === 预定环节（v10.11 简化：2026-06-23 用户决定）
    # 合并：预定异常 / 询价异常-* / 询价失败-* / 证件异常 / 验价失败 → 统一归"预定环节"
    # 保留独立族（业务动作指引不同）：亏损过大 / 政策亏损 / 未匹配规则 / 渠道账号为空 / 价格不符 / 利润过大 / 订单已完结
    # 渠道账号为空
    (r"获取.*账号为空", "预定环节-渠道账号为空"),
    # 价格不符（自家价格）
    (r"自家价格", "预定环节-价格不符"),
    ...
    (r"订单已完结", "预定环节-订单已完结"),
    # 政策亏损（保留渠道）
    (r"反采同程.*亏损大于", "预定环节-亏损过大-反采同程"),
    (r"反采携程海外.*亏损大于", "预定环节-亏损过大-反采携程海外"),
    (r"反采同程.*亏损", "预定环节-政策亏损-反采同程"),
    (r"反采携程海外.*亏损", "预定环节-政策亏损-反采携程海外"),
    (r".*亏损大于", "预定环节-亏损过大-其他渠道"),
    # 未匹配自动规则（多个变体）
    (r"没有匹配到自动出票规则|未找到出票规则|初筛没有匹配出票规则|未匹配自动规则", "预定环节-未匹配规则"),
    # 兜底：所有"预定失败/预订失败/验价失败/证件异常/询价失败/询价异常"统一归"预定环节"
    (r"预定失败|预订失败|验价失败|证件异常|询价失败|询价异常", "预定环节"),

    # === 支付环节（细分：通用支付环节 / 订单校验失败）===
    (r"订单校验失败", "支付环节-订单校验失败"),
    (r"支付失败", "支付环节-支付异常"),
...
def get_stage_only(family: str) -> str:
    """2026-07-26 修复：把"族-子分类"提取成"8 大环节 + 兜底"，与前端 STAGE_ORDER 对齐。
    ...
    固定 9 族（8 大环节 + 兜底）：
        人工环节 / 系统环节 / 预定环节 / 支付环节 / 取票环节
        / 回填环节 / 验真环节 / 平台环节 / 其他环节
    输出必须含"环节"二字，与前端 dashboard_v5.html STAGE_ORDER + getStageColor 完全对齐。

    历史 bug：之前剥掉"环节"二字返回"预定/支付/取票..."，
    前端 groupByStage() 的 STAGE_ORDER（"X环节"）匹配不上，
    导致所有族归到"其他环节"兜底 → 8 大环节汇总条失效 / 族级色编码全青色。
    """
    if not family:
        return "其他环节"
    # split("-")[0] 直接就是"预定环节/取票环节"等（已含"环节"二字）
    base = family.split("-")[0]
    if base in ("人工环节", "系统环节", "预定环节", "支付环节",
                "取票环节", "回填环节", "验真环节", "平台环节"):
        return base
    return "其他环节"
...
FAMILY_SUB_NORMALIZE_RULES = {
    ...
    # 三大头：
    #   - `预定环节 原因为:` 前缀（光 594 单变体一个）→ 删
    #   - `预定环节 ` 短前缀（"未找到预定服务:xxx"/"无有效预留号码:xxx"）→ 删
    ...
    "预定环节-预定异常": [
        ...
        # === v10 新增（2026-06-22）===
        # 1) 删 `预定环节 原因为:` 长前缀（覆盖绝大多数变体，最大头）
        (r"^预定环节\s*原因为[:：]\s*", ""),
        # 2) 删 `预定环节 ` 短前缀（覆盖 "未找到预定服务:xxx" / "无有效预留号码:xxx" 等不带"原因为:"的变体）
        (r"^预定环节\s+", ""),
        ...
        (r"\[回调消息\]\s*预定环节[:：]该订单需要护照号", "需要护照号 (回调消息)"),
        ...
        (r"\[回调消息\]\s*支付环节\s*考虑银行卡问题\s*人工核实[:：]\s*支付未完成", "支付环节 银行卡问题 (回调消息)"),
    ...
    "预定环节-渠道账号为空": [
        ...
        # 归一为 "账号为空"（去掉渠道名差异）
        (r"获取[^\s]+账号为空", "账号为空"),
        # 2) 长文本 "预定环节 原因为:询价失败:未获取到价格反采同程APP 反采同程询价异常 获取反采同程APP账号为空 反采同程APP"
        # 删空冗余前缀，剩 "账号为空 [渠道名]"
        (r"^预定环节\s*原因为[:：]\s*询价失败[:：]\s*未获取到价格[^\s]+\s*[^\s]+询价异常\s*", ""),
    ],

    # === 取票环节族 ===
    "取票环节-取票异常": [
        # "取票环节 当前状态为:" (191+32) vs "取票环节 当前订单状态为:" vs "当前订单状态为:" 归一
        # 注意：原 cleaned 字符串可能以"取票环节 原因为:"开头，"取票环节 当前状态为:"在中间
        (r"取票环节\s*当前(订单)?状态为[:：]\s*", "当前订单状态为: "),
        # 各种子状态归一
        (r"暂时没有票号", ""),
        (r"订单状态为空", ""),
        (r"请求错误未查询到订单信息", ""),
    ...
    "支付环节-支付异常": [
        ...
        # "打开网页失败 报错原因..." 删空
        (r"打开网页失败\s*报错原因[^,，:：]+", ""),
        # "支付环节:insufficient balance" 删空
        (r"支付环节[:：]?insufficient balance", ""),
        # "请人工核实支付宝6039" 数字删空
...
def family_sub_normalize(reason: str, family: str) -> str:
    ...
    含义：
    - 族聚合（REASON_FAMILY_RULES）已经把"相似的字样"归到同一族（如"预定环节-预定异常"），
      这是用户要的"合并"。
    - 族内的具体变体（如"预定环节 原因为:反采同程" / "预定环节 原因为:内部程序异常 反采同程"）
...
STAGE_RULES = [
    # ============ 预定（包含失信/证件/实名） ============
    (lambda s: "预定环节" in s or "预订失败" in s
    ...
    # ============ 支付 ============
    (lambda s: "支付环节" in s or "支付超时" in s or "支付时超时" in s
              or "支付时" in s, "支付-支付环节"),
    ...
    (lambda s: "待出票" in s or "VALIDATING" in s
              or "验证中" in s, "取票-待出票"),
    (lambda s: "取票环节" in s and "订单已支付" in s, "取票-状态异常"),
    (lambda s: "取票环节" in s and "订单关闭" in s, "取票-订单已关闭"),
    (lambda s: "取票环节" in s and "订单状态为空" in s, "取票-状态为空"),
    (lambda s: "取票环节" in s and "ip异常" in s, "取票-IP异常"),
    (lambda s: "取票环节" in s and ("令牌" in s or "登录已过期" in s), "取票-凭证异常"),
    (lambda s: "取票环节" in s and ("JsonParseException" in s or "Unexpected char" in s), "取票-请求异常"),
    (lambda s: "取票环节" in s and "取消订单失败" in s, "取票-取消失败"),
    (lambda s: "取票环节" in s and "退票中" in s, "取票-退票中"),
    (lambda s: "取票环节" in s and "未查询到订单信息" in s, "取票-未查到订单"),
    (lambda s: "取票环节" in s, "取票-其他环节"),  # 兜底
...
def build_month_data(df, month_label):
    ...
    # 2026-07-20 修复：自然周聚合（移到 out["daily"] = daily 之后，之前位置 out["daily"] 还没赋值，整段被跳过）
    ...
    #   4) 族聚合：把相似根因归到业务族（"取票环节"/"询价失败"/"亏损"等）

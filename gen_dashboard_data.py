...
REASON_FAMILY_RULES = [
    ...
    # 订单处理超时
    # v10.x（2026-07-26）："开卡"匹配必须在 daily maintenance window 之前
    #   原因：reason 实际长这样"开卡失败 原因:daily maintenance window (23:55-00:05)"
    #   如果 daily maintenance window 先匹配, 这 67 单会归"系统环节-定时维护"——错
    #   实际是"支付环节-虚拟卡失败"（夜间维护导致开卡失败, 是支付前置步骤）
    (r"开卡失败|开卡成功.*查询详情失败|issue card error", "支付环节-虚拟卡失败"),
    ...
    # 卡余额不足（CNH 是卡余额不足的英文, "开卡失败/issue card" 已挪到"支付环节-虚拟卡失败"）
    (r"CNH bal not enough", "系统环节-卡余额不足"),
    # 接口异常（航旅纵横 LY0502）
    ...
    # v10.x (2026-07-26): 扩展"证件"类, 覆盖"证件有效期/证件格式/证件类型"等变体
    #   背景: 14 单"同程ITravel解析乘客信息异常证件有效期解析异常" 散在"其他环节-兜底"
    #   实际是预定环节的证件解析异常 (用户反馈)
    (r"预定失败|预订失败|验价失败|证件异常|证件有效期|证件格式|证件类型|询价失败|询价异常", "预定环节"),
    ...
    # v10.x (2026-07-26): 代码报错/内部错误/程序异常 → 系统环节-代码错误 (预防性族规则, 当前 0 单匹配)
    #   用户反馈: "代码报错应该定义成系统环节"
...
FAMILY_SUB_NORMALIZE_RULES = {
    ...
    # === 验真环节族 ===
    # v10.x（2026-07-26）：验真异常 99 单变体太多（"-:9C 客票验真 乘客 · 报错: Read timed out" 等），
    #   跨航司同类错误（Read timed out / Server disconnected / 未知错误）应该合并。
    #   用户反馈"航司一致+报错一致的可以归类到一起" → 启用族内归一
    #   注意: 仅"验真环节-验真异常"族启用, 其他族保持 v10.5 行为 (不归一, 保留变体)
    "验真环节-验真异常": [
        # 1) 主要 pattern: "验真异常 -:航司 客票验真 乘客 · 报错: 原因" / "验真异常 -:航司 客票验真 乘客: 报错: 原因"
        #    例: "验真异常 -:MF 客票验真 乘客: 报错: Read timed out" → "验真异常 报错: Read timed out"
        #    航司支持数字+字母 (9C/7C) / 纯字母 (VJ/MF/HX) / 数字-字母 (731-MF)
        #    注: "乘客" 和 "报错" 中间可能是 " · " / ": " / " " 等任意分隔
        #    注: 还有 "验真异常 -731-:MF 客票验真" (数字+冒号+航司) 的变体
        (r"^验真异常\s+-:?(\d+-:)?(\d*-)?[A-Z0-9]{1,3}[:：]?\s*客票验真\s*·?\s*乘客[\s:：·]*·?\s*报错[:：]?\s*", "验真异常 报错: "),
        # 2) 没航司的变体: "验真异常 客票验真 乘客 · 报错: 原因" / "验真异常 客票验真 乘客: 报错: 原因"
        (r"^验真异常\s+客票验真\s*·?\s*乘客[\s:：·]*·?\s*报错[:：]?\s*", "验真异常 报错: "),
        # 3) 带订单号的变体: "验真异常 -5CF4S2: 客票验真 乘客 · 报错: 原因"
        (r"^验真异常\s+-[\w]+:[\w]*\s*客票验真\s*·?\s*乘客[\s:：·]*·?\s*报错[:：]?\s*", "验真异常 报错: "),
        # 4) 兜底: "验真异常 -:航司 客票验真 乘客: 原因" (没"· 报错:"中间段)
        (r"^验真异常\s+-:?(\d+-:)?(\d*-)?[A-Z0-9]{1,3}[:：]?\s*客票验真\s*·?\s*乘客[:：]?\s*", "验真异常 "),
        # 5) 订单号+直接报错: "验真异常 -5CF4S2: 证件号不符" / "验真异常 -731-:xxx"
        (r"^验真异常\s+-[\w]+:[\w]*[:：]?\s*", "验真异常 "),
        # 6) 兜底: 删"-:航司"或"-: " (如 "验真异常 -: 未知错误" → "验真异常 未知错误")
        (r"^验真异常\s+-:?\s*[A-Z0-9]{1,3}[:：]?\s*", "验真异常 "),
        # 7) 没"-"但有航司的: "验真异常 MF 客票验真 乘客 · 报错: 原因" / "验真异常 客票验真 乘客: 报错: 原因"
        (r"^验真异常\s+[A-Z0-9]{1,3}\s*客票验真\s*·?\s*乘客[\s:：·]*·?\s*报错[:：]?\s*", "验真异常 报错: "),
        # 8) 兜底: 删"-: "或"-:" 任何非字母数字前缀 (兜底, 上面规则匹配不到时)
        (r"^验真异常\s+-:?\s*", "验真异常 "),
    ],

    # === 支付环节族 ===
    "支付环节-虚拟卡失败": [
        # "开卡成功 查询详情失败 卡号:原因:卡详情获取为空,请重试" → "开卡成功 查询详情失败"
        (r"开卡成功\s*查询详情失败\s*卡号[:：]\s*原因[:：][^,，]+", "开卡成功 查询详情失败"),
        # "开卡成功 查询详情失败 卡号:xxx" → "开卡成功 查询详情失败"（卡号噪音删空）
        (r"开卡成功\s*查询详情失败\s*卡号[:：][^\s,，:：]+", "开卡成功 查询详情失败"),
        # "开卡失败 原因:Cannot invoke..." / "开卡失败 原因:system error" → 统一归"开卡失败"
        (r"开卡失败\s*原因[:：].*", "开卡失败"),
        # "开卡失败" + daily maintenance window 整段归一
        (r"开卡失败\s*原因[:：]daily maintenance window[^,，]*", "开卡失败"),
    ],
    "支付环节-支付异常": [
        # "获取支付结果失败人工核实" 删空
        (r"获取支付结果失败人工核实", ""),
...
def family_sub_normalize(reason: str, family: str) -> str:
    ...
    设计变更（2026-07-26）：
    - v10.5（2026-06-22）起族内归一被禁用（保留变体原始信息，让业务看清全貌）
    - v10.x（2026-07-26）部分恢复族内归一：仅"验真环节-验真异常"族启用
      原因: 99 单验真异常变体太多（"-:9C 客票验真 乘客 · 报错: Read timed out" 跨航司同类重复），
           用户反馈"航司一致+报错一致的可以归类到一起"
    - 其他族保持 v10.5 行为: 不归一, 保留变体原始信息
    ...
    - 例外: 验真环节变体太多（"航司+客票验真+乘客+报错"组合），族内归一让"Read timed out"跨航司合并
    """
    s = reason.strip()
    # 仅"验真环节-验真异常"族启用族内归一
    if family == "验真环节-验真异常":
        rules = FAMILY_SUB_NORMALIZE_RULES.get(family, [])
...
def build_month_data(df, month_label, is_daily=False):
    """对单月数据跑所有聚合，返回该月的 dict。
    v10.15（2026-08-17）：is_daily=True 时不归并"其他"长尾桶（用户拍板：单日数据全展开）
    """
...
def _main_inner(args):
    ...
    for m in target_months:
        ...
        for date in sorted(df_m["_file_date"].unique()):
            ...
            if len(df_day) == 0:
                continue
            day_data = build_month_data(df_day, date, is_daily=True)  # v10.15 单日不归并"其他"
            # 精简：去掉单日用不到的字段
